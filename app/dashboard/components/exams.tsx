...
import { getAllAssessmentsExam } from "@/app/redux/assessments/assessment-slice";
...
export default function Exams() {
  ...
  const { assessmentExam } = useAppSelector((x) => x.assessment);
  ...
  const getStatusCount = (status: string) => {
    return (
      assessmentExam?.summary?.[
        status as keyof typeof assessmentExam.summary
      ] || 0
    );
  };
      ...
      <div className="flex-1 overflow-auto no-scrollbar">
        <table className="w-full">
          ...
          <tbody>
            ...
            ) : assessmentExam && assessmentExam.assessments.length > 0 ? (
              assessmentExam.assessments.map((exam) => {
                const statusKey =
